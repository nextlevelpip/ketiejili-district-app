...
// FIXED: Added FileText to the import list
import { Shield, ShieldCheck, Users, Search, PhoneCall, Building2, Target, UserPlus, CheckCircle2, AlertCircle, Loader2, Trash2, FileText } from 'lucide-react';
...
export default function Presbytery() {
  ...
  // FIXED: Provide ALL active members for leadership appointments
                      ...
                      {displayedAppointments.map(a => (
                        <tr key={a.id} className="hover:bg-[#001D3D]/50 transition-colors">
                          ...
                          <td className="p-5 text-center">
                            <button onClick={() => handleRemoveAppointment(a.id, a.memberName)} className="p-2 text-white/30 hover:bg-red-500/20 hover:text-red-400 rounded-lg transition-colors" title="Revoke Appointment">
                              <Trash2 size={16} />
                            </button>
                          </td>
                        </tr>
                      ))}
                      {displayedAppointments.length === 0 && <tr><td colSpan="6" className="p-10 text-center text-white/50 font-bold italic text-xs">No leaders found. Time to appoint your council!</td></tr>}
                    </tbody>
                  </table>
                </div>
              </div>
