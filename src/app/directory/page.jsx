      ...
      <div className="min-h-full rounded-[2.5rem] bg-gradient-to-br from-[#1e3a8a] via-[#1e40af] to-[#172554] p-6 md:p-10 text-white relative overflow-hidden shadow-2xl pb-20">
        ...
        <div className="relative z-10 max-w-7xl mx-auto space-y-6 animate-fade-in">
          ...
          <div className="flex justify-center items-center gap-4 mb-8">
            <button onClick={() => { resetForm(); setActiveTab('register'); }} className={`px-6 py-3 rounded-[14px] font-bold flex items-center gap-2 transition-all border backdrop-blur-md ${activeTab === 'register' ? 'bg-white/20 text-white border-white/30 shadow-lg' : 'bg-black/20 text-blue-200 hover:text-white hover:bg-white/10 border-white/10'}`}>
              <UserPlus size={18}/> {editingId ? 'Edit Record' : 'Register Member'}
            </button>
            <button onClick={() => setActiveTab('directory')} className={`px-6 py-3 rounded-[14px] font-bold flex items-center gap-2 transition-all border backdrop-blur-md ${activeTab === 'directory' ? 'bg-white/20 text-white border-white/30 shadow-lg' : 'bg-black/20 text-blue-200 hover:text-white hover:bg-white/10 border-white/10'}`}>
              <Users size={18}/> Member Directory ({members.length})
            </button>
          </div>

          {activeTab === 'register' ? (
            <div className="bg-white/10 backdrop-blur-xl p-8 md:p-10 rounded-3xl shadow-xl border border-white/10 max-w-5xl mx-auto">
              <form onSubmit={handleSave} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <select required value={memberStatus} onChange={e => setMemberStatus(e.target.value)} className={`${inputStyle} border-emerald-400/30 bg-emerald-900/20`}>
                ...
                {churchRole === 'New Convert' && (
                  <div className="bg-orange-500/20 border border-orange-500/30 p-6 rounded-2xl grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in mt-2 backdrop-blur-sm">
                    <div>
                      <label className={`${labelStyle} text-orange-200 flex items-center gap-1`}><Flame size={12}/> Soul Winning Category</label>
                      <select value={soulWinner} onChange={e => setSoulWinner(e.target.value)} className={`${inputStyle} border-orange-400/30 bg-black/40 text-orange-100`}>
                        <option value="">- Select Origin / Category -</option>
                        {specializedSoulCategories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className={`${labelStyle} text-orange-200`}>Water Baptized?</label>
                      <select value={waterBaptized} onChange={e => setWaterBaptized(e.target.value)} className={`${inputStyle} border-orange-400/30 bg-black/40 text-orange-100`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-black/20 border border-white/5 p-6 rounded-2xl mt-4 backdrop-blur-md">
                  <div>
                    <label className={labelStyle}>Any Disabilities?</label>
                    <select value={hasDisability} onChange={e => setHasDisability(e.target.value)} className={inputStyle}>
                  ...
                  {hasDisability === 'Yes' && (
                    <div className="md:col-span-2 animate-fade-in">
                      ...
                      <select required value={disabilityType} onChange={e => setDisabilityType(e.target.value)} className={inputStyle}>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/10 backdrop-blur-xl p-4 md:p-6 rounded-[2rem] shadow-xl border border-white/10 grid grid-cols-1 md:grid-cols-5 xl:grid-cols-6 gap-3">
                <div className="relative xl:col-span-2">
                  <Search className="absolute left-4 top-3.5 text-blue-200/50" size={16}/>
                  <input placeholder="Search directory..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 p-3 bg-black/20 border border-white/10 rounded-xl font-bold text-sm outline-none focus:border-blue-400 text-white placeholder:text-blue-200/50" />
                </div>
                
                <select value={fStatus} onChange={e => setFStatus(e.target.value)} className="p-3 bg-black/20 border border-white/10 rounded-xl font-bold text-xs outline-none focus:border-blue-400 text-white [&>option]:text-gray-900">
                ...
                <select value={fRole} onChange={e => setFRole(e.target.value)} className="p-3 bg-black/20 border border-white/10 rounded-xl font-bold text-xs outline-none focus:border-blue-400 text-white [&>option]:text-gray-900">
                  <option value="All Roles">All Roles</option>
                  {churchRoles.map(r => <option key={r} value={r}>{r}</option>)}
                </select>
                
                <select value={fAssem} onChange={e => setFAssem(e.target.value)} className="p-3 bg-black/20 border border-white/10 rounded-xl font-bold text-xs outline-none focus:border-blue-400 text-white [&>option]:text-gray-900">
                ...
                <select value={fDemo} onChange={e => setFDemo(e.target.value)} className="p-3 bg-black/20 border border-white/10 rounded-xl font-bold text-xs outline-none focus:border-blue-400 text-white [&>option]:text-gray-900">
              ...
              <div className="bg-white/10 backdrop-blur-xl rounded-[2rem] shadow-xl border border-white/10 overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    <thead>
                      <tr className="bg-black/20 border-b border-white/10 text-[10px] font-black text-blue-200 uppercase tracking-widest">
                          ...
                          <tr key={m.id} className={`transition-colors ${isInactive ? 'bg-black/10 opacity-60 hover:opacity-100' : 'hover:bg-white/5'}`}>
                            <td className="p-5">
                              ...
                              <div className="flex gap-2 items-center mt-1">
                                ...
                                {isInactive && <span className="text-[9px] font-black text-red-200 uppercase tracking-wider bg-red-500/20 px-2 py-0.5 rounded border border-red-400/30">{m.memberStatus}</span>}
                              </div>
                            </td>
                            <td className="p-5">
                            ...
                            <td className="p-5">
                              <div className="font-bold text-white/80">{m.gender}</div>
                            ...
                            <td className="p-5">
                              <div className="flex justify-center gap-2">
                                <button onClick={() => handleEdit(m)} className="p-2 text-blue-300 bg-blue-500/20 hover:bg-blue-500/40 border border-blue-400/30 hover:text-white rounded-xl transition-all"><Edit3 size={16}/></button>
                                <button onClick={() => handleDelete(m.id, m.name)} className="p-2 text-red-300 bg-red-500/20 hover:bg-red-500/40 border border-red-400/30 hover:text-white rounded-xl transition-all"><Trash2 size={16}/></button>
                              </div>
                            </td>
                          </tr>
                        );
